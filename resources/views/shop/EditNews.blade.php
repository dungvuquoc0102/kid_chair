<?php include('shopheader.php') ?>
            <div class="d-flex justify-content-center align-items-center text-center p-2" style="width: 100%">
                <fieldset class='bg-dark text-light w-50 p-3'>
                    <legend class="text-center text-warning">Sửa tin tức</legend>
                    <form action='/EditNews' method='post' enctype="multipart/form-data">
                        @csrf
                        <div style='text-align:center'>
                            <img id='avatar' src="{{URL::asset($news->image)}}" alt="Đã thêm ảnh" style='width:50%;height:50%;'>
                        </div>
                        <div style='text-align:center'>
                        ...
                        <div class="d-flex flex-column justify-content-between">
                            <input name="news_id" type="hidden" value="{{$news->news_id}}"/>
                            <div class="row border border-secondary p-1 m-1">
                                <label class="col-3">Tiêu đề</label>
                                <input class="col-9" type='text' name='title' value="{{$news->title}}" required>
                            </div>
                            <div class="row border border-secondary p-1 m-1">
                                <label class="col-3">Tóm tắt</label>
                                <input class="col-9" type='text' name='summary' value="{{$news->summary}}" required>
                            </div>
                            <div class="row border border-secondary p-1 m-1">
                                ...
                                <select id="category" class="col-9" type='text' name='category_id' required>
                                    ...
                                    foreach($categories as $category){
                                        if($category->category_id == $news->category_id){
                                ...
                                <select id="sub_category" class="col-9" type='text' name='sub_category_id'>
                                    <?php
                                        foreach($sub_categories as $sub_category){
                                            if($sub_category->sub_category_id == $news->sub_category_id){
